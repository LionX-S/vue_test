...
$\quad$ 1.被用来给元素或自组建注册引用信息（id的替代者）

$\quad$ 2.应用在html标签上获取的是真实的DOM元素，应用在组件标签上是组件实例对象（vc）
...
$\quad\quad$ 第三种方式（限制类型、限制必要性、指定默认值）：
...
$\quad$ 3.每个组件都有自己的```$route```属性，里面存储这自己的路由信息。
...
$\quad\quad$ 1.activated路由组件被激活时触发。

$\quad\quad$ 2.deactivated路由组件失活时触发。
